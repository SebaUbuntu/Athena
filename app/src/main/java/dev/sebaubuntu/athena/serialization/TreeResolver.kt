...
class TreeResolver(
    ...
    private suspend fun Resource.serialize(): Any? = when (this) {
        is Element -> when (isNavigable) {
            true -> {
                addResourceIdentifierToResolve(identifier)
                null
            }
    ...
    private suspend fun Iterable<Element>.serialize() = mapAsync { element ->
        element.serialize()?.let {
            element.identifier.path.last() to it
        }
