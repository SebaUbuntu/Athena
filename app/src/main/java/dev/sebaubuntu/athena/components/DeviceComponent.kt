...
class DeviceComponent(context: Context) : Component {
    class Factory : Component.Factory {
        override fun create(context: Context) = DeviceComponent(context)
    }

    private val activityManager = context.getSystemService(ActivityManager::class.java)

    private val memoryInfo = ActivityManager.MemoryInfo().apply {
        activityManager.getMemoryInfo(this)
    }

    override val name = "device"

    override val title = LocalizedString(R.string.section_device_name)

    override val description = LocalizedString(R.string.section_device_description)

    override val drawableResId = R.drawable.ic_perm_device_information
                ...
                elements = listOfNotNull(
                    Element.Card(
                        ...
                        elements = listOf(
                            ...
                            Element.Item(
                                identifier = identifier / "general" / "hardware_name",
                                title = LocalizedString(R.string.device_hardware_name),
                                value = Value(Build.HARDWARE),
                            ),
                            Element.Item(
                                identifier = identifier / "general" / "board_name",
                    ...
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
                        Element.Card(
                            ...
                            elements = listOf(
                                Element.Item(
                                    identifier = identifier / "sku" / "hardware_sku",
                                    title = LocalizedString(R.string.device_hardware_sku),
                                    value = Value(Build.SKU),
                                ),
                                Element.Item(
                                    identifier = identifier / "sku" / "odm_sku",
