...
class CpuComponent : Component {
    class Factory : Component.Factory {
        override fun create(context: Context) = CpuComponent()
    }

    override val name = "cpu"

    override val title = LocalizedString(R.string.section_cpu_name)

    override val description = LocalizedString(R.string.section_cpu_description)

    override val drawableResId = R.drawable.ic_developer_board
            ...
            val screen = Screen.CardListScreen(
                ...
                elements = listOf(
                    ...
                    Element.Card(
                        ...
                        title = LocalizedString(R.string.cpu_general),
                        elements = listOfNotNull(
                            CpuInfoUtils.getProcessors().takeIf { it.isNotEmpty() }?.let {
                                Element.Item(
                                    identifier = identifier / "processors",
                            ...
                            CpuInfoUtils.getL2Caches().takeIf { it.isNotEmpty() }?.let {
                                Element.Item(
                                    identifier = identifier / "l2_caches",
                            ...
                            CpuInfoUtils.getL3Caches().takeIf { it.isNotEmpty() }?.let {
                                Element.Item(
                                    identifier = identifier / "l3_caches",
                            ...
                            CpuInfoUtils.getL4Caches().takeIf { it.isNotEmpty() }?.let {
                                Element.Item(
                                    identifier = identifier / "l4_caches",
            ...
            null -> pollFlow {
                ...
                val screen = Screen.ItemListScreen(
                    ...
                    elements = clusters.map {
                        Element.Item(
                            identifier = identifier / "${it.clusterId}",
                            ...
                            elements = listOfNotNull(
                                Element.Card(
                                    ...
                                    elements = listOfNotNull(
                                        ...
                                        Element.Item(
                                            identifier = identifier / "general" / "uarch",
                                            title = LocalizedString(R.string.cpu_uarch),
                                            value = Value(cluster.uarch),
                                        ),
                                        cluster.cpuid?.let {
                                            Element.Item(
                                                identifier = identifier / "general" / "cpuid",
                                                title = LocalizedString(R.string.cpu_cpuid),
                                                value = Value(it),
                                            )
                                        },
                                        Element.Item(
                                            identifier = identifier / "general" / "frequency",
            ...
            null -> pollFlow {
                ...
                val screen = Screen.ItemListScreen(
                    ...
                    elements = cores.map { core ->
                        Element.Item(
                            identifier = identifier / "${core.coreId}",
                            title = LocalizedString(
                            ...
                            elements = listOfNotNull(
                                Element.Card(
                                    ...
                                    elements = listOfNotNull(
                                        ...
                                        Element.Item(
                                            identifier = identifier / "general" / "core_id",
                                            title = LocalizedString(R.string.cpu_core_id),
                                            value = Value(core.coreId),
                                        ),
                                        Element.Item(
                                            identifier = identifier / "clusters" / "${core.cluster.clusterId}",
                                            title = LocalizedString(R.string.cpu_cluster_id),
                            ...
                            elements = listOf(
                                Element.Card(
                                    ...
                                    elements = listOf(
                                        ...
                                        Element.Item(
                                            identifier = identifier / "general" / "cluster_start",
                                            title = LocalizedString(R.string.cpu_cluster_start),
                                            value = Value(value.clusterStart),
                                        ),
                                        Element.Item(
                                            identifier = identifier / "general" / "cluster_count",
            ...
            null -> pollFlow {
                ...
                val screen = Screen.ItemListScreen(
                    ...
                    elements = processors.map {
                        Element.Item(
                            identifier = identifier / "${it.linuxId}",
                            title = LocalizedString(
                        ...
                        Screen.CardListScreen(
                            ...
                            elements = listOf(
                                Element.Card(
                                    ...
                                    elements = listOfNotNull(
                                        Element.Item(
                                            identifier = identifier / "general" / "smt_id",
                                ...
                                Element.Card(
                                    ...
                                    elements = listOfNotNull(
                                        linuxCpu.isOnline?.let {
                                            Element.Item(
                                                identifier = identifier / "status" / "is_online",
                                        ...
                                        linuxCpu.currentFrequencyHz?.let { currentFrequencyHz ->
                                            Element.Item(
                                                identifier = identifier / "status" / "current_frequency_hz",
                                        ...
                                        linuxCpu.minimumFrequencyHz?.let { minimumFrequencyHz ->
                                            Element.Item(
                                                identifier = identifier / "status" / "minimum_frequency_hz",
                                        ...
                                        linuxCpu.maximumFrequencyHz?.let { maximumFrequencyHz ->
                                            Element.Item(
                                                identifier = identifier / "status" / "maximum_frequency_hz",
                                        ...
                                        linuxCpu.scalingCurrentFrequencyHz?.let { scalingCurrentFrequencyHz ->
                                            Element.Item(
                                                identifier = identifier / "status" / "scaling_current_frequency_hz",
                                        ...
                                        linuxCpu.scalingMinimumFrequencyHz?.let { scalingMinimumFrequencyHz ->
                                            Element.Item(
                                                identifier = identifier / "status" / "scaling_minimum_frequency_hz",
                                        ...
                                        linuxCpu.scalingMaximumFrequencyHz?.let { scalingMaximumFrequencyHz ->
                                            Element.Item(
                                                identifier = identifier / "status" / "scaling_maximum_frequency_hz",
    ...
    private fun Midr.getCardElement(
        identifier: Resource.Identifier,
    ) = Element.Card(
        identifier = identifier,
        ...
        elements = listOf(
            Element.Item(
                identifier = identifier / "implementer",
